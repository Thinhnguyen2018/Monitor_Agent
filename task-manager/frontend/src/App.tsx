...
const App: React.FC = () => {
  ...
  // Roadmap needs Milestone & Release rows but still respects search/module/status filters for tasks
  ...
  const handleCellChange = useCallback(
    async (id: number, fields: Partial<Pick<Task, 'module' | 'quarter' | 'month' | 'week'>>) => {
    ...
    <div className="app">
      ...
      <header className="header">
        ...
        <nav className="header-tabs">
          ...
          <button
            className={`tab-btn${tab === 'milestones' ? ' active' : ''}`}
...
<button className="btn btn-primary" onClick={handleNewTask}>
      ...
      {/* Filter bar — hidden on milestones tab */}
      {tab !== 'milestones' && tab !== 'dashboard' && (
        <FilterBar
          search={search}
          onSearch={setSearch}
