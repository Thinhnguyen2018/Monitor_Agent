...
const taskColumn = (task: Task, granularity: Granularity, viewMonth?: number): string | null => {
  ...
  // week: only show tasks in the selected month
  const m = task.month ?? quarterFirstMonth[task.quarter]
...
const RoadmapCell: React.FC<{
  ...
  module: ModuleKey
  tasks: Task[]
  onStatusCycle: (id: number, s: Task['status']) => void
  isToday?: boolean
    ...
    <td ref={setNodeRef} className={cls}>
      {isEmpty ? (
        ...
        <div className="roadmap-bars">
          {tasks.map((t) => (
            <MilestoneMarker key={t.id} task={t} onClick={() => onStatusCycle(t.id, t.status)} />
          ))}
        </div>
      ) : module === 'Release' ? (
        <div className="roadmap-bars">
          {tasks.map((t) => (
            <ReleaseTag key={t.id} task={t} onClick={() => onStatusCycle(t.id, t.status)} />
          ))}
        </div>
      ) : (
        <div className="roadmap-bars">
          {tasks.map((t) => (
            <RoadmapBar key={t.id} task={t} onStatusCycle={() => onStatusCycle(t.id, t.status)} />
          ))}
        </div>
      )}
    </td>
...
const OverlayBar: React.FC<{ task: Task }> = ({ task }) => {
  if (task.module === 'Milestone') return (
    <div className="milestone-marker roadmap-bar-overlay">
      <span className="milestone-diamond">◆</span>
      <span className="milestone-label">{task.title}</span>
    </div>
  )
  if (task.module === 'Release') return (
    <div className="release-tag roadmap-bar-overlay">
      <span className="release-icon">🚀</span>
      <span className="release-label">{task.title}</span>
    </div>
  )
...
const Roadmap: React.FC<RoadmapProps> = ({ tasks, onStatusCycle, onCellChange }) => {
  ...
  // for week view: which month are we looking at
  ...
  const handleExport = async (format: 'png' | 'pdf') => {
    ...
    setExporting(true)
    try {
      // Target the <table> directly so we get full dimensions, not the clipped scroll container
      const tableEl = tableRef.current.querySelector('table') ?? tableRef.current

      // Temporarily expand scroll container so html2canvas sees the full content
  ...
  // Build column descriptors per granularity
  const columns: Array<{ id: string; label: string; group?: string }> = (() => {
    if (granularity === 'quarter') return QUARTERS.map((q) => ({ id: q, label: q }))
    if (granularity === 'month') return MONTH_NAMES.map((name, i) => ({
      id: String(i + 1),
      label: name,
      group: monthToQuarter(i + 1),
    }))
    // week: 4 weeks for the selected month
  ...
  const todayQuarter = monthToQuarter(todayMonth)
  const todayColId = granularity === 'quarter'
    ? todayQuarter
    : granularity === 'month'
    ? String(todayMonth)
    : (viewMonth === todayMonth ? String(todayWeekOfMonth) : null)

  // Group headers for month view (Q1/Q2/Q3/Q4 spanning 3 cols each)
  const showGroupHeader = granularity === 'month'

  const getTasksForCell = (module: ModuleKey, colId: string) =>
    tasks.filter((t) => {
      if (t.module !== module) return false
      const col = taskColumn(t, granularity, viewMonth)
      return col === colId
    })
    ...
    <div className="roadmap-wrapper">
      ...
      <div className="roadmap-toolbar">
        ...
        <div className="export-btns">
          <button className="btn btn-ghost export-btn" onClick={() => handleExport('png')} disabled={exporting} title="Export as PNG">
            {exporting ? '…' : '⬇ PNG'}
          </button>
          <button className="btn btn-ghost export-btn" onClick={() => handleExport('pdf')} disabled={exporting} title="Export as PDF">
            {exporting ? '…' : '⬇ PDF'}
          </button>
        </div>
        ...
        {granularity === 'week' && (
          <div className="month-nav">
            ...
            <span className="month-nav-label">
              {MONTH_NAMES[viewMonth - 1]} 2026 · {monthToQuarter(viewMonth)}
            </span>
            <button
              className="month-nav-btn"
      ...
      <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
          ...
          <table className="roadmap-table">
            <thead>
              {showGroupHeader && (
                <tr>
                  ...
                  {QUARTERS.map((q) => {
                    ...
                    return (
                      <th key={q} colSpan={end - s + 1} className="roadmap-quarter-group">
                        {q}
                      </th>
                    )
                  })}
              ...
              <tr>
                ...
                {columns.map((col) => {
                  const isToday = col.id === todayColId
                  return (
                    <th key={col.id} className={isToday ? 'roadmap-th-today' : ''}>
                      {col.label}
              ...
              {MODULES.map((module) => (
                <tr key={module}>
                  ...
                  {columns.map((col) => (
                    <RoadmapCell
                      key={col.id}
                      cellId={`${module}__${col.id}`}
                      module={module}
                      tasks={getTasksForCell(module, col.id)}
                      onStatusCycle={onStatusCycle}
                      isToday={col.id === todayColId}
                    />
                  ))}
                </tr>
              ))}
        ...
        <DragOverlay>
          {dragging ? <OverlayBar task={dragging} /> : null}
        </DragOverlay>
      </DndContext>
    </div>
  )
